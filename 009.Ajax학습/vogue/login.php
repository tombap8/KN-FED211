...
<body>
    ...
    <!-- 
        ...
        - method 속성 : 전송방식 선택(get/post)
            -> get방식은 url로 데이터를 전달하는 방식
            -> post방식은 페이지로 숨겨서 데이터를 전달하는 방식
            (보안성은 post방식이 좋다!)
        ...
        - type 속성: 여러가지 입력양식을 선택함
            ...
            8) 기타 html5에서 새로이 등장한 type들
            
            - search : 검색, tel : 전화번호, email : 이메일주소
              url : 웹페이지 주소
            -> 기존에 text 하나만 있었으나 데이터 유효성검사를
            브라우저 자체 내장 유효성 검사를 할 수 있게 제공함!
            ...
            2. placeholder 속성 : 입력창에 안내문구를 출력
                                (입력순간 글자는 사라짐!)
            3. value 속성 : 미리 입력하고자 하는 텍스트를 넣음
                            (버튼일 경우 버튼 글자로 출력됨!)
            4. disabled 속성 : 값없이 바로 속성명만 사용해서
                            입력금지상태로 만든다!
            ...
            - for속성 : input 요소의 id명과 같아서 라벨클릭시
            입력요소로 포커스가 자동으로 들어간다!
